...
export async function onRequestGet(context){
  ...
  const {content}=await ghRead('employees');
...
export async function onRequestPost(context){
  ...
  const {content:employees,sha}=await ghRead('employees');
  let list = employees||[];
  ...
  if(action==='create'){
    ...
    list.push(emp);
    const ok = await ghWrite('employees',list,sha);
  ...
  if(action==='replace_all'){
    const ok = await ghWrite('employees',body.employees,sha);
    return respond({ok});
  }
