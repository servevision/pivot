...
function genEmployeeId(list){
  let max=0;
  list.forEach(e=>{const m=(e.employeeId||'').match(/HSIM-EMP-(\d+)/);if(m)max=Math.max(max,parseInt(m[1]));}); 
  return 'HSIM-EMP-'+String(max+1).padStart(3,'0');
}

async function sendEmail(resendKey,to,subject,html){
  if(!resendKey)return false;
  try{
    const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{'Authorization':`Bearer ${resendKey}`,'Content-Type':'application/json'},body:JSON.stringify({from:'HSIM India <onboarding@resend.dev>',to:[to],subject,html})});
    return r.ok;
...
export async function onRequestPost(context){
  ...
  if(decision==='approve'){
    ...
    for(let attempt=0; attempt<5; attempt++){
      const {content:employees,sha:empSha}=await ghRead('hsim-employees');
      ...
      ok = await ghWrite('hsim-employees',empList,empSha);
      if(ok) break;
    ...
    const {content:logins,sha:loginSha}=await ghRead('hsim-employee-logins');
    ...
    await sendEmail(env.RESEND_API_KEY,reqItem.email,
      'Your HSIM India account has been approved ✓',
      `<div style="font-family:sans-serif;max-width:480px">
        <h2 style="color:#0a5239">Welcome to HSIM India! 🎉</h2>
        <p>Hi ${reqItem.name},</p>
        <p>Your account has been <b>approved</b>.</p>
        <p><b>Employee ID:</b> ${employeeId}</p>
        <p>You can now login to the employee portal.</p>
        <a href="https://pivot-eb5.pages.dev/hsim-employee.html" style="background:#1758a8;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block;margin-top:10px">Login to Portal →</a>
      </div>`
    );
  ...
  if(decision==='reject'){
    ...
    await sendEmail(env.RESEND_API_KEY,reqItem.email,
      ...
      `<div style="font-family:sans-serif;max-width:480px">
        ...
        <p>Hi ${reqItem.name}, your request was not approved. Please contact HSIM admin.</p>
      </div>`
    );
