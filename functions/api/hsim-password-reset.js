...
export async function onRequestGet(context){
  ...
  const {content} = await ghRead('hsim-password-reset-requests');
...
export async function onRequestPost(context){
  ...
  if(action==='request'){
    ...
    if(info){
      const {content:reqs, sha} = await ghRead('hsim-password-reset-requests');
      const list = reqs||[];
      ...
      if(!already){
        ...
        await ghWrite('hsim-password-reset-requests', list, sha);
      }
    }
  ...
  if(action==='resolve'){
    ...
    const {content:reqs, sha:reqSha} = await ghRead('hsim-password-reset-requests');
    const list = reqs||[];
    ...
    await ghWrite('hsim-password-reset-requests', list, reqSha);

    await sendEmail(env.RESEND_API_KEY, list[idx].email,
      'Your HSIM India password has been reset',
      `<div style="font-family:sans-serif;max-width:480px">
        ...
        <a href="https://pivot-eb5.pages.dev/hsim-employee.html" style="background:#1758a8;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block;margin-top:10px">Login →</a>
      </div>`
    );
